...
from DAD.model import DAD
from DAD.config import GlobalConfig
...
class DinoAgent(autonomous_agent.AutonomousAgent):
	def setup(self, path_to_conf_file):
		...
		self._im_transform = T.Compose([T.ToTensor()])
	...
	def run_step(self, input_data, timestamp):
		...
		control.steer = np.clip(steer_traj, -1, 1)
		control.throttle = np.clip(throttle_traj, 0, 0.75)
		control.brake = np.clip(brake_traj, 0, 1)
